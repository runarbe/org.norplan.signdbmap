...
var signDbMap = (function() {
    ...
    /**
     * Function to call whenever point is clicked
     * 
     * @param  {leaflet.feature} feature [description]
     * @param  {leaflet.layer} layer   [description]
     * @ignore
     */
    function onEachFeature(feature, layer) {
        layer.on('click', function(e) {
            console.log('Clicked :' + feature.properties.NAME_EN + '(' + feature.properties.ABBREV + ')');
        });
    }
     ...
     *
     * @param {String} districtsJsonFile Filename of geojson file containing districts
     */
    m.addDistricts = function(districtsJsonFile) {

        console.log('Called addDistricts function');

        jQuery.getJSON(districtsJsonFile, {}, function(json, textStatus) {

            if (isInit()) {
                console.log('Loaded json' + JSON.stringify(json));

                var districts = L.geoJson(json, {
                    style: {
                        "color": "black",
                        "opacity": 0.25
                    },
            ...
            console.log('Request failed:' + t + ' - ' + e);
        });

    };

    /**
     * Always to be the last item of the enclosure.
     */
    return m;
